...
const createSensor = async (req, res) => {
    try {
        const sensor = new Sensor(sanitize(req.body));
        await sensor.save();
...
const getSensorById = async (req, res) => {
    try {
        const sensor = await Sensor.findById(sanitize(req.params.id));
...
const updateSensor = async (req, res) => {
    try {
        const sensor = await Sensor.findByIdAndUpdate(sanitize(req.params.id), sanitize(req.body), {
            new:
                true
        });
        if (!sensor) return res.status(404).json({ msg: 'Sensor no encontrado' });
...
const deleteSensor = async (req, res) => {
    try {
        const sensor = await Sensor.findByIdAndDelete(sanitize(req.params.id));
        if (!sensor) return res.status(404).json({ msg: 'Sensor no encontrado' });
